...
export async function GET(
	...
	const fileExists = data?.find((file) => file.name === `${patient.cid}.png`);
	...
	const signaturePictureURL = db.storage
		.from("pictures")
		.getPublicUrl(`user-signatures/${patient.cid}.png`).data.publicUrl;

	const title = `דוח קליטת מטופל`;
		...
		>
			...
			<section id="receiever-info">
				...
				<span>
					...
					<br />
					{`${getDateString(createdAtDate)} ${getTimeString(
						createdAtDate
					)}`}
				</span>

				<img
					src={`${signaturePictureURL}?${Date.now()}`}
					alt="receiver signature"
				/>
			</section>
			<FullLogoIcon id="watermark" />
		</PDFPage>
	);
	...
	/* 	const res = await fetch(
		"https://mirpa-a-external-printer.vercel.app/api/report",
		{
			method: "POST",
			body: JSON.stringify({
				htmlString: "<p>asdasdasd</p>",
			}),
			cache: "no-store",
		}
	); */
	...
	await fetch("https://mirpa-a-external-printer.vercel.app/api/report", {
		method: "POST",
		body: JSON.stringify({
			htmlString,
		}),
	...
	const response = new Response(stream, {
		headers: {
			...
			"Content-Disposition": `inline; file="patient-reception-report.pdf"; filename="${encodeURIComponent(
				filename
			).replace(/%0A|%09/g, "")}.pdf"`,
